    ...
    for fp in sorted(Path(path).glob("*.txt")):
        ...
        if variants:
            ...
            out[pid] = ScoreDefinition(pid, hdr.get("trait_reported", "unknown"),
                                       # Coordinates come from hm_pos when the file is harmonised, and the
                                       # build of THOSE coordinates is #HmPOS_build; #genome_build keeps the
                                       # original submission's build (often "NR"). Checking genome_build while
                                       # using hm_pos compares a different build from the one in play (Manuel, R14).
...
def load_population_af(path: Path) -> "AFTable":
    ...
    if out and not any("_allele" in e for e in out.values()):
        ...
        # The warning has to travel with the number, not only to a terminal nobody keeps.
        # A gnomAD export that counts the other allele flips the sign of every printed shift,
        # and the clinician reading sd_note is the person who needs to know that (Manuel, R14).
        # Carried as an attribute, not a dict key: the table is keyed by rsid and its length is
        # reported to the clinician as af_rows, so a sentinel key would silently inflate that count.
        out.orientation_unverified = True
    return out
...
def af_shift(score: ScoreDefinition, af_table: dict[str, dict[str, Any]],
    ...
    if orientation_unverified:
        note = (note + " " if note else "") + (
            ...
            "unverified: it was assumed to be this score's effect allele. A table counting the "
            "other allele would reverse the sign of this shift.")
    return AFShift(
