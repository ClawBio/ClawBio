...
class TestRound9Sweep:
    ...
    def test_af_table_with_allele_column_makes_no_orientation_claim(self, tmp_path):
        import prs_abstain as pa
        f = tmp_path / "af_with_allele.tsv"
        f.write_text("rs1\tAFR\t0.40\tA\nrs2\tAFR\t0.10\tC\n")
        tab = pa.load_population_af(f)
        assert tab.orientation_unverified is False
        score = pa.ScoreDefinition("PGSTEST", "trait", "GRCh37", [
            {"rsid": "rs1", "effect_allele": "A", "other_allele": "G", "weight": 0.5, "af_reference": 0.10},
            {"rsid": "rs2", "effect_allele": "C", "other_allele": "T", "weight": 0.5, "af_reference": 0.05},
        ])
        sh = pa.af_shift(score, tab, sd=1.0, population="AFR")
        assert sh is not None and "unverified" not in sh.sd_note.lower()

    def test_harmonised_file_declares_the_build_of_the_coordinates_it_uses(self, tmp_path):
        """Manuel, R14: #genome_build keeps the SUBMITTED build (often NR); the coordinates we
        read come from hm_pos, whose build is #HmPOS_build. Checking the first while using the
        second compares a build that is not in play."""
        import prs_abstain as pa
        f = tmp_path / "PGS999999.txt"
        f.write_text(
            "#pgs_id=PGS999999\n#trait_reported=Test\n#genome_build=NR\n#HmPOS_build=GRCh38\n"
            "rsID\tchr_name\tchr_position\thm_pos\teffect_allele\tother_allele\teffect_weight\n"
            "rs1\t1\t100\t200\tA\tG\t0.1\n")
        defs = pa.load_score_definitions(tmp_path)
        assert defs["PGS999999"].build == "GRCh38", (
            "the build checked must be the build of the coordinates actually used")
