...
def _prompt_field(s: dict, idx: int) -> dict:
    ...
    # Keep cli_alias default in sync if name changed
    if key == "name" and not s.get("cli_alias"):
        s["cli_alias"] = re.sub(r"[^a-z0-9]", "", s["name"].lower())
...
def _md_deps(deps: dict) -> str:
    ...
    out = "**Required** (in `requirements.txt` or skill-level install):\n"
    if req:
        out += "\n".join(f"- `{d}` — TODO: purpose" for d in req)
    else:
        out += "- Python 3.11+ standard library only — no external packages"
    if opt:
        out += "\n\n**Optional**:\n"
        out += "\n".join(f"- `{d}` — TODO: purpose (graceful degradation without it)" for d in opt)
    return out
...
def patch_clawbio_py(clawbio_path: Path, spec: dict) -> bool:
    ...
    Returns True on success, False if alias already exists or anchor not found.
    """
    try:
    ...
    if f'"{cli_alias}"' in source and "SKILLS_DIR" in source:
        print(f"  {DIM}clawbio.py: alias '{cli_alias}' already present — skipping.{RESET}")
        return False

    # Find the anchor: the line right after the last SKILLS entry, just before
    # "# Skills that run in the full-profile pipeline"
    anchor = "# Skills that run in the full-profile pipeline"
    if anchor not in source:
        print(
            f"  {RED}clawbio.py: could not find anchor '{anchor}' — skipping auto-patch.{RESET}",
            file=sys.stderr,
        )
        return False
    ...
    patched = source.replace(anchor, f"{indented}\n{anchor}")
...
def scaffold_skill(
    ...
    # 6. Update catalog.json + clawbio.py if repo is found
    catalog_updated   = False
    clawbio_updated   = False
    if repo_root and not dry_run:
        catalog_path  = repo_root / "skills" / "catalog.json"
        clawbio_path  = repo_root / "clawbio.py"
        catalog_entry = generate_catalog_entry(spec)

        if catalog_path.exists():
            catalog_updated = update_catalog_json(catalog_path, catalog_entry)
        else:
...
def write_skill_builder_report(
    ...
    if not manifest.get("catalog_updated"):
        next_steps.append(
            "1. Add the catalog entry: append `catalog_entry.json` to `skills/catalog.json`"
        )
    if not manifest.get("clawbio_updated"):
        next_steps.append(
            f"2. Add to `clawbio.py` SKILLS dict — see `examples/example_spec.json` for the entry"
        )
    next_steps.append(
        f"3. Fill in all `TODO:` placeholders in `skills/{name}/SKILL.md` and `skills/{name}/{name.replace('-','_')}.py`"
    )
    next_steps.append(
        f"4. Run tests: `python -m pytest skills/{name}/tests/ -v`"
    )
    next_steps.append(
        f"5. Submit PR: `git checkout -b add-{name} && git add skills/{name}/ && git commit -m 'Add {name} skill' && git push`"
    )
...
def main() -> None:
    ...
    if not args.dry_run:
        ...
        # Write skill-builder's own output report into a subdirectory
        sb_report_dir = output_base / f"_skill_builder_report_{spec['name']}"
